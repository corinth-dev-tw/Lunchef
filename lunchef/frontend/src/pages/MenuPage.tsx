...
export default function MenuPage() {
  ...
  useEffect(() => {
    if (restaurant) {
      ...
      if (passed) {
        setCutoffWarning(t('menu.cutoffWarning', { date: isToday ? t('menu.today') : orderDate, time: restaurant.order_cutoff_time }))
      } else {
        setCutoffWarning('')
      }
    }
  }, [restaurant, orderDate, t, isToday])
  ...
  const fetchRestaurantDetails = async () => {
    try {
      ...
      setError(err.message || t('menu.failedLoadRestaurant'))
    }
  }
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      setError(err.message || t('menu.failedLoadMenu'))
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="px-4 py-2 flex items-center justify-between bg-white border-b">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">
              {isToday ? t('menu.today') : orderDate}
            </span>
          </div>
          <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full font-medium">
            {t('menu.orderBy')} {restaurant?.order_cutoff_time}
          </span>
        </div>
      </header>
              ...
              <div className="flex">
                ...
                <div className="flex-1 p-3 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-base font-bold text-green-600">{formatTwd(item.price)}</p>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-[60] px-4 py-3 safe-area-pb">
          <div className="flex justify-between items-center mb-2">
            <div>
              <span className="font-bold text-gray-800">{t('menu.itemsCount', { count: getCartItemCount() })}</span>
