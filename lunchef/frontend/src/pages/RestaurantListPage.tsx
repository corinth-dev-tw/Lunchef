  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <header className="bg-white shadow-sm p-4">
        ...
        <h1 className="text-xl font-bold text-gray-800">{t('restaurant.title')}</h1>
      </header>
      ...
      <div className="p-4 space-y-4">
        {restaurants.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <p className="text-gray-600">{t('restaurant.noRestaurants')}</p>
          </div>
        ) : (
              ...
              <div className="h-36 bg-gray-100 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className="text-xs bg-white/90 backdrop-blur-sm text-gray-700 px-2.5 py-1 rounded-full font-medium shadow-sm">
                    {t('restaurant.orderBy', { time: restaurant.order_cutoff_time })}
                  </span>
                </div>
              </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      {t(`home.cuisines.${restaurant.cuisine_type || 'all'}`)} · {restaurant.department_store} {restaurant.floor}
                    </p>
                  </div>
                  <span className="text-xs bg-orange-50 text-orange-700 px-2.5 py-1 rounded-full font-medium whitespace-nowrap ml-2">
                    {t('restaurant.minOrder')} {restaurant.min_order_type === 'items'
