...
import { formatTwDate, getRelativeDayLabel } from '../i18n/formatters'
...
const CUISINE_COLORS: Record<string, string> = {
  ...
  thai: 'bg-orange-100 text-orange-800',
  ...
  asian: 'bg-purple-100 text-purple-800',
}

function getCuisineInfo(type: string, translate: TranslateFn) {
  return {
    label: (translate as any)(`home.cuisines.${type}`) || translate('home.cuisines.asian'),
    color: CUISINE_COLORS[type] || CUISINE_COLORS.asian,
  }
}
...
function getDateLabel(dateStr: string, translate: TranslateFn): string {
  const rel = getRelativeDayLabel(dateStr)
  if (rel === 'today') return translate('home.today')
  if (rel === 'tomorrow') return translate('home.tomorrow')
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-50">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">{t('app.name')}</h1>
          <p className="text-gray-600">{t('login.subtitle')}</p>
        </div>
        <div className="w-full max-w-sm bg-white rounded-lg shadow-md p-6">
          <p className="text-center text-gray-600 mb-6">{t('login.lineNote')}</p>
          <button
            onClick={login}
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs text-gray-500 font-medium">{t('home.selectLocation')}</p>
            <p className="text-sm font-bold text-gray-800 truncate">
              {selectedLocation ? selectedLocation.name : t('home.selectLocation')}
            </p>
          </div>
        ...
        {selectedLocation && (
          <div className="bg-white rounded-xl border border-gray-200 p-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-xs text-gray-500 font-medium mb-1">{t('home.selectDate')}</p>
        ...
        {selectedLocation && restaurants.length > 0 && (
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4">
            {availableCuisines.map((type) => {
              const info = getCuisineInfo(type, t)
        ...
        <div className="space-y-3">
          {!selectedLocation ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
              ...
              <p className="text-gray-600 font-medium mb-2">{t('home.selectLocation')}</p>
              <p className="text-gray-400 text-sm mb-6">{t('home.noRestaurants')}</p>
              <button
                onClick={() => setShowLocationPicker(true)}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
              ...
              <p className="text-gray-600 font-medium mb-1">
                {searchQuery ? t('home.noRestaurantsForCuisine') : t('home.noRestaurants')}
              </p>
            </div>
          ) : (
            restaurants.map((restaurant) => {
              const cuisine = getCuisineInfo(restaurant.cuisine_type || 'asian', t)
              return (
                <button
                  key={restaurant.id}
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex gap-2 flex-wrap">
                      <span className="text-xs bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full font-medium inline-flex items-center gap-1">
                        ...
                        {t('restaurant.orderCutoff')} {restaurant.order_cutoff_time}
                      </span>
                      <span className="text-xs bg-orange-50 text-orange-700 px-2.5 py-1 rounded-full font-medium inline-flex items-center gap-1">
                        ...
                        {restaurant.min_order_type === 'items'
                          ? t('restaurant.minOrderItems', { count: restaurant.min_order_value })
                          : t('restaurant.minOrderAmount', { amount: restaurant.min_order_value })}
                      </span>
                    </div>
                  </div>
                </button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl max-h-[70vh] flex flex-col animate-slide-up">
            <div className="p-4 border-b flex justify-between items-center">
              <h2 className="text-lg font-bold">{t('home.selectLocation')}</h2>
              <button
